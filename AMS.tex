...
\section{Semiconductor process and device basics}
    ...
    \begin{itemize}
        ...
        \item \textbf{CMOS Cross-Section:} NMOS built in P-substrate (with N+ S/D). PMOS built in N-well (with P+ S/D). Both have poly gates over thin oxide.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Cascading Systems:} Total gain $H_{tot}(s) = A(s) \cdot B(s)$. In Bode plots, magnitudes in dB are added ($|A|_{dB} + |B|_{dB}$) and phases are added. To maximize overall broadband gain, the corner frequency $\omega_c$ of cascaded stages should be placed as high as possible.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Capacitor mismatch ($1\sigma$):} $\sigma_C/C\approx \frac{K_C}{\sqrt{WL}}$.
        \item \textbf{Improve Matching:} Increase capacitor area, use unit capacitors, add dummy elements at edges, use common-centroid or interdigitated layouts, and add shielding.
...
\section{Amplifiers}
    ...
    \begin{itemize}
        ...
        \item \textbf{Voltage Buffer (Unity-Gain):} Connect output directly to the inverting input ($V_{out} = V_{in-}$).
    ...
    \begin{itemize}
        ...
        \[ A_{cm}=\frac{v_{od}}{v_{icm}},\quad \mathrm{CMRR}=\left|\frac{A_{vd}}{A_{cm}}\right| \]
        \[ \mathrm{CMRR}_{dB}=20\log_{10}(\mathrm{CMRR}) \]
        \item Input common-mode range (resistive-load pair):
        \[ V_{in,CM,min}\approx V_{GS}+V_{DS,sat,tail} \]
        \[ V_{in,CM,max}\approx V_{out,CM}-V_{DS,sat}-V_{GS} \]
...
\section{References \& Mixed-Signal Blocks}
    ...
    \subsection{Power Management (LDO \& DC-DC)}
    \begin{itemize}
        ...
        \item \textbf{High-Efficiency Step-Down:} For large voltage steps (e.g., $5\text{V} \to 3.1\text{V}$), Inductor-based DC-DC converters are much more efficient than LDOs or charge pumps.
    ...
    \begin{itemize}
        \item \textbf{Ring Oscillator:} Formed by cascaded inverting stages. To sustain oscillation, the total number of stages $N$ must be an odd number.
        \item \textbf{Period Calculation:} The time for a signal to complete a full cycle ($0 \to 1 \to 0$) is given by:
        \[ T_{osc} = 2 \cdot N \cdot T_{delay} \]
        where $T_{delay}$ is the propagation delay of a single inverter stage.
    \end{itemize}
